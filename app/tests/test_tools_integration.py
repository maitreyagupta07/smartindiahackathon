...
from app.router.model_registry import TEXT_MODEL, LORA_ADAPTER
...
async def test_doc_search_flow_calls_tool_then_qwen():
    ...
    with patch("app.agent.loop.search_docs", new=AsyncMock(return_value=tool_result)) as mocked_tool, \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value="Fire watch is required for hot work.")) as mocked_infer:
        resp = await run_agent_loop(req)
...
async def test_document_generation_flow_prepares_content_via_qwen_before_generate_file():
    ...
    with patch("app.agent.loop.generate_file", new=AsyncMock(return_value=tool_result)) as mocked_tool, \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value=lora_raw_output)) as mocked_infer:
        resp = await run_agent_loop(req)
...
async def test_approval_note_flow_uses_lora_adapter_for_content_prep():
    ...
    with patch("app.agent.loop.generate_file", new=AsyncMock(return_value=tool_result)), \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value=json.dumps(prepared_content))) as mocked_infer:
        resp = await run_agent_loop(req)
...
async def test_tool_failure_retries_then_finalizes_with_error():
    ...
    with patch(
        "app.agent.loop.execute_code", new=AsyncMock(side_effect=RuntimeError("docker unavailable"))
    ) as mocked_tool, patch(
        "app.agent.loop.call_inference", new=AsyncMock(return_value="```python\nprint(5 * 5)\n```")
    ):
        resp = await run_agent_loop(req)
...
async def test_tool_retry_succeeds_on_second_attempt():
    ...
    with patch(
        "app.agent.loop.search_docs",
        new=AsyncMock(side_effect=[RuntimeError("timeout"), {"results": []}]),
    ) as mocked_tool, patch(
        "app.agent.loop.call_inference", new=AsyncMock(return_value="No CUI info found.")
    ):
        resp = await run_agent_loop(req)
