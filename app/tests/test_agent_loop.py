...
from app.router.model_registry import TEXT_MODEL, VISION_MODEL
...
async def test_text_only_routes_to_qwen_single_step():
    ...
    with patch(
        "app.agent.loop.call_inference", new=AsyncMock(return_value="a qwen text response")
    ) as mocked:
...
async def test_plain_image_routes_to_moondream_single_step():
    ...
    with patch(
        "app.agent.loop.call_inference", new=AsyncMock(return_value="a photo of a valve")
    ) as mocked:
        resp = await run_agent_loop(req)
...
async def test_image_plus_reasoning_chains_moondream_then_qwen():
    ...
    with patch(
        "app.agent.loop.call_inference", new=AsyncMock(side_effect=responses)
    ) as mocked:
        resp = await run_agent_loop(req)
