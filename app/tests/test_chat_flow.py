...
from app.router.model_registry import TEXT_MODEL
...
async def test_chat_flow_retrieves_scoped_docs_and_uses_history():
    ...
    with patch("app.agent.loop.search_docs", new=AsyncMock(return_value=tool_result)) as mocked_tool, \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value="They save planning time and avoid shutdowns.")) as mocked_infer:
...
async def test_chat_flow_without_history_still_answers():
    ...
    with patch("app.agent.loop.search_docs", new=AsyncMock(return_value=tool_result)), \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value="It's a vessel inspection report.")) as mocked_infer:
        resp = await run_agent_loop(req)
...
async def test_chat_flow_no_matching_docs_is_honest():
    ...
    with patch("app.agent.loop.search_docs", new=AsyncMock(return_value={"results": []})), \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value="No information on turbines in the uploaded documents.")) as mocked_infer:
        resp = await run_agent_loop(req)
...
async def test_chat_isolation_each_chat_only_sees_its_own_chat_id():
    ...
    with patch("app.agent.loop.search_docs", new=fake_search), \
         patch("app.agent.loop.call_inference", new=AsyncMock(return_value="ok")):
...
async def test_non_chat_request_is_unchanged():
    ...
    with patch("app.agent.loop.call_inference", new=AsyncMock(return_value="a summary")) as mocked_infer:
        resp = await run_agent_loop(req)
