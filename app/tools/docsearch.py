...
from ..storage.config import REPO_ROOT, FILES_DIR
...
def _get_client():
    """
    The single shared PersistentClient for both collections (corpus + chat
    KB). anonymized_telemetry=False is not optional here — Chroma's default
    Settings phone home to its own servers on every add/query
    (CollectionAddEvent/CollectionQueryEvent) unless explicitly disabled,
    which would otherwise be a real, silent violation of the air-gapped/
    no-external-calls requirement. Found while validating the single-node
    refactor; pre-existing, not something this refactor introduced.
    """
    global _client
    if _client is None:
